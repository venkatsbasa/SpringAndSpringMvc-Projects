...
public class EmpDaoImpl extends JdbcDaoSupport implements EmpDao {
	...
	public void createEmployee(Employee emp) {
		...
		int update=getJdbcTemplate().update(Sql,new Object[] {emp.getEmpName(),emp.getGender(),emp.getEmail()});
		if(update>0) {
	...
	public Employee getEmployeeById(int empId) {
		...
		return getJdbcTemplate().queryForObject(Sql, new EmpRowMapper(), empId);
	}

	@Override
	public void deleteEmployeeById(int empId) {
		 String Sql="delete from venkats.EMPLOYEE_TABLE where emp_id=?";
		 if( getJdbcTemplate().update(Sql, empId)>0) {
    ...
    if( getJdbcTemplate().update(Sql, email,empid)>0) {
