...
public class EmpDaoImpl extends JdbcDaoSupport implements EmpDao {
	 ...
	 */
	/*
	 * @Override public void createEmployee(Employee emp) { String
	 * Sql="insert into venkats.EMPLOYEE_TABLE(emp_name,emp_gender,emp_email) values(?,?,?)"
	 * ; int update=jdbcTemplate.update(Sql,new Object[]
	 * {emp.getEmpName(),emp.getGender(),emp.getEmail()}); if(update>0) {
	...
	public Employee getEmployeeById(int empId) {
		...
		return null;// jdbcTemplate.queryForObject(Sql, new EmpRowMapper(), empId);
	}

	@Override
	public void deleteEmployeeById(int empId) {
		/*
		 * String Sql="delete from venkats.EMPLOYEE_TABLE where emp_id=?"; if(
		 * jdbcTemplate.update(Sql, empId)>0) {
	...
	public List<Employee> getEmpInfo() {
		...
		return empList;// jdbcTemplate.query(Sql, new EmpRowMapper());
	}
